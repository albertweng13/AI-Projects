    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                allPossible[p] = hmm[p] * emissionModel[trueDistance]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particle_count = self.numParticles
        legal_positions = self.legalPositions
        position_list = [None] * particle_count

        index = 0
        while particle_count > 0:
            for elem in legal_positions:
                if particle_count == 0:
                    break
                position_list[index] = elem
                index += 1
                particle_count -= 1

        self.beliefs = position_list
    ...
    def observe(self, observation, gameState):
        ...
        hmm = self.beliefs
        num_of_particles = len(hmm)
        jail = self.getJailPosition()

        jail_list = []
        if noisyDistance == None:
            for elem in range(num_of_particles):
                jail_list.append(jail)
            self.beliefs = jail_list

        else:
            allPossible = util.Counter()
            for elem in hmm:
                trueDistance = util.manhattanDistance(elem, pacmanPosition)
                allPossible[elem] += emissionModel[trueDistance]
            ...
            if allPossible.totalCount() == 0:
                ...
                final_list = []
                for elem in range(num_of_particles):
                    new_value = util.sample(allPossible)
                    final_list.append(new_value)

                self.beliefs = final_list
    ...
    def elapseTime(self, gameState):
        ...
        new_list = []
        for elem in hmm:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, elem))
            new_value = util.sample(newPosDist)
            new_list.append(new_value)

        self.beliefs = new_list
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legal_positions = self.legalPositions
        position_list = [] # We append the tuples to this list

        cartesian = itertools.product(legal_positions, repeat=num_ghosts)

        while num_of_particles > 0:
            for elem in cartesian:
                if num_of_particles == 0:
                    break
                position_list.append(elem) #Not sure if this is random...
                num_of_particles -= 1
            cartesian = itertools.product(legal_positions, repeat=num_ghosts)
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                ...
                for particle in range(num_of_particles):
                    hmm[particle] = self.getParticleWithGhostInJail(hmm[particle], i)

        for elem in hmm:
            temp_product = 1.0
            for i in range(self.numGhosts):
                if i in eaten_ghosts:
                    continue
                trueDistance = util.manhattanDistance(elem[i], pacmanPosition)
                temp_product = temp_product * emissionModels[i][trueDistance]
            allPossible[elem] += temp_product

        if allPossible.totalCount() == 0:
            self.initializeParticles()
            for particle in range(len(self.beliefs)):
                for index in eaten_ghosts:
                    self.beliefs[particle] = self.getParticleWithGhostInJail(self.beliefs[particle], index)
