        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.beliefs:
            ...
            for newPos in newPosDist:
                ...
                jointprob = newPosprob*oldPosprob

                allPossible[newPos] += jointprob
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        combos = list(itertools.product(legal, repeat=numGhosts))
        ...
        for count in range(num):
            swag = count % numCombos
            if (swag == 0):
                random.shuffle(combos)
            plist.append(tuple(combos[swag]))
    ...
    def observeState(self, gameState):
        ...
        #ghosts in prison cell
        prisoners = []
        ghosty = 0
        while (ghosty < numGhosts):
            if (noisyDistances[ghosty] is None and ghosty not in prisoners):
                prisoners.append(ghosty)
            ghosty += 1
        ...
        for particle in self.particles:
            ...
            while (ghosty < numGhosts):
                ...
                #place ghost in prison for every particle if captured once
                if (ghosty in prisoners):
                    particle = self.getParticleWithGhostInJail(particle, ghosty)

                #chase ghost and try to place in prison
                else:
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[ghosty])
                    emissionModel = emissionModels[ghosty]
                    prob = emissionModel[trueDistance]
                    result *= prob

                ghosty += 1
            allPossible[particle] += result
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            numGhosts = self.numGhosts
            for ghosty in range(numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), ghosty, self.ghostAgents[ghosty])
                newParticle[ghosty] = util.sample(newPosDist)
