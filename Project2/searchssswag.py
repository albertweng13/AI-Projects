...
def positionLogicPlan(problem):
    ...
    # Start at startState
    nonStartStates = []
    pacmanStart = logic.PropSymbolExpr("P", startState[0], startState[1], finalTime)
    index_x = 1
    for x in range(1, problem.getWidth()+1):
        for y in range(1, problem.getHeight()+1):
            if (x,y) == (startState[0], startState[1]):
                ...
                nonStartStates.append(logic.PropSymbolExpr("P", x, y, finalTime))
    wow = logic.to_cnf(pacmanStart & ~(logic.associate('|', nonStartStates)))
    cnf.append(wow)
    #cnf.append(logic.PropSymbolExpr("P", startState[0], startState[1], finalTime))
    #successor axioms, use BFS
    q = util.Queue()
    q.push((startState,0, startState))
    visited = []
    goal_states = []
    while not q.isEmpty():
        current = q.pop()
        if current[0] in visited:
            ...
            for successor in successorList:
                successorlogic =  (logic.PropSymbolExpr('P',successor[0][0],successor[0][1], current[1]+1)) >> (logic.PropSymbolExpr("P", current[0][0], current[0][1], current[1]) & logic.PropSymbolExpr(successor[1],current[1]))
                cnf.append(logic.to_cnf(successorlogic))
                if problem.terminalTest(successor[0]):
                    visited.append(successor[0])
                    finalTime = current[1] + 1
                q.push((successor[0], current[1]+1, current[0]))
            if problem.terminalTest(successor[0]):
                break
    ...
    for time in range(finalTime+1):
        ...
        cnf.append(logic.to_cnf(oneturnLogic))
