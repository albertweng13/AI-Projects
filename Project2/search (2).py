...
def positionLogicPlan(problem):
    ...
    for t in range(1, t_max+1):
        ...
        """for i in range(len(state_exprs)):
            for j in range(i+1, len(state_exprs)):
                if (state_exprs[i][0] == state_exprs[j][0]) & (i != j):
                    new_start = state_exprs[i][0]
                    new_tuple = state_exprs[i][1] | state_exprs[j][1]
                    state_exprs.remove(state_exprs[j])
                    state_exprs.remove(state_exprs[i])
                    state_exprs.append((new_start, new_tuple))"""
