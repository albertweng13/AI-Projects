...
def enhancedFeatureExtractorDigit(datum):
    ...
    y = 0
    y_constant = y
    while (x < (width * 65/100)):
        while (y < (height * 33/100)):
            if datum.getPixel(x, y) > 0:
                features[str(pixel)] = 1
            else:
                features[str(pixel)] = 0
            y += 1
            pixel += 1
        x += 1
        y = y_constant
    ...
    b = height * 33/100
    b_constant = y
    while (a < (width * 65/100)):
        while (b < (height * 66/100)):
            if datum.getPixel(a, b) > 0:
                features[str(pixel)] = 1
            else:
                features[str(pixel)] = 0
            b += 1
    ...
    n = height * 35/100
    n_constant = n
    while (m < (width * 35/100)):
        while (n < (height * 65/100)):
            if datum.getPixel(i, j) > 0:
                features[str(pixel)] = 1
            else:
                features[str(pixel)] = 0
    ...
    t = height * 35/100
    t_constant = t
    while (s < (width * 99/100)):
        while (t < (height * 65/100)):
            if datum.getPixel(i, j) > 0:
                features[str(pixel)] = 1
            else:
                features[str(pixel)] = 0
...
def enhancedPacmanFeatures(state, action):
    ...
    """foodCount = successor.getFood().count()
    features['foodCount'] = 1/(foodCount + 1)"""
    ...
    for x in range(state.data.layout.width):
        for y in range(state.data.layout.height):
            if (foodGrid[x][y]):
                distance = util.manhattanDistance(successor.getPacmanPosition(), (x,y))
                features[str(foodInt)] = 1/distance
                foodInt += 1
    ...
    for ghost in ghosts:
        ghostDistance = util.manhattanDistance(successor.getPacmanPosition(), ghost)
        if (ghostDistance < closestGhost):
            closestGhost = ghostDistance
    if (closestGhost == 0):
        features['closestGhost'] = sys.maxint
    else:
        features['closestGhost'] = 1/closestGhost"""

    """numGhosts = successor.getNumAgents()
    features['numGhosts'] = 1/(numGhosts - 1)"""

    """closestCapsule = sys.maxint
    for capsule in successor.getCapsules():
        capsuleDistance = util.manhattanDistance(successor.getPacmanPosition(), capsule)
        if (capsuleDistance < closestCapsule):
            closestCapsule = capsuleDistance
    if (closestCapsule != sys.maxint):
