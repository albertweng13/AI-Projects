...
def enhancedFeatureExtractorDigit(datum):
    ...
    while (not queue.isEmpty()):
        ...
        for elem in array:
            if ((elem[0] < 0) or (elem[1] < 0) or (elem[0] > 27) or (elem[1] > 27)) or ((elem[0], elem[1]) in visited) or ((elem[0], elem[1]) in in_queue):
                continue
            elif datum.getPixel(elem[0], elem[1]) == 0:
                queue.push((elem[0], elem[1]))
                in_queue.add((elem[0], elem[1]))
        visited.add((x, y))
...
def enhancedPacmanFeatures(state, action):
    ...
    capsuleList = successor.getFood().count()
    ...
    features['foodCount'] = capsuleList
